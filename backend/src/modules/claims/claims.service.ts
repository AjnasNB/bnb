  ...
  async findOne(id: string) {
    try {
      ...
      if (claim) {
        ...
        // Try to get additional blockchain data (optional)
        try {
          ...
          // Continue with database claim only
        }
      ...
      // If not in database, try blockchain (optional)
      try {
        const blockchainClaim = await this.contractService.getClaimDetails(id);
        if (blockchainClaim) {
      ...
      // If not found anywhere, provide fallback claim data for voting
